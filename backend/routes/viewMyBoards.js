...
router.post("/" + apiName, async(req, res) => {
  try {
    ...
    console.log(userId)
    let resBoards = (await client.query("SELECT a.id, a.name, b.can_manage_users, b.can_manage_tasks, b.can_manage_board FROM boards a JOIN user_to_board b ON a.id = b.board_id WHERE b.user_id = $1", [userId])).rows
